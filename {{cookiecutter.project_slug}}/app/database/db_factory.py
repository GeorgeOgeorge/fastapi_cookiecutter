...
class DatabaseManager:
    ...
    async def _create_pool_connection(self, db_pool: Engine) -> Session:
        ...
        try:
            ...
            logger.error(f"error while creating database connection from pool: {connection_error}")
            raise connection_error
        except Exception as generic_error:
            logger.error(f"error raise while creating database connection from pool: {generic_error}")
            raise generic_error
        else:
            return new_session
